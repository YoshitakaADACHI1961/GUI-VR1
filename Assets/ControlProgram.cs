...
public class Control_Info2
{
    ...
    // 文字列からJSONデータを作る
    public static Control_Info2 CreateFromJSON(string jsonString)
    {
        return JsonUtility.FromJson<Control_Info2>(jsonString);
    }
...
public class RaspControl2
{
    ...
    // 文字列からJSONデータを作る
    public static RaspControl2 CreateFromJSON(string jsonString)
    {
        return JsonUtility.FromJson<RaspControl2>(jsonString);
    }
...
{
    Control_Info2 InitialParam = new Control_Info2(); // 初期化用ファイルから入力されるパラメータ
    RaspControl2 ControlParam = new RaspControl2(); // Raspberry Pi の制御用
    ...
    void Start()
    {
        ...
        if (File.Exists(initFile)) // ファイルが存在する場合
        {
            ...
            try
            {
                ...
                InitialParam = JsonUtility.FromJson<Control_Info2>(datastr);
                reader.Close();
            }
      ...
      UnityEngine.Application.Quit();
        ...
        try
        {
            ...
            Debug.Log("動画用　Raspberry Pi 制御パラメータファイルの書き込み");
        }
        catch (System.IO.IOException ex)
        {
            Debug.Log("動画用　制御パラメータファイルの書き込みでエラーになりました" + InitialParam.ParamFile + "  ERROR:" + ex);
        }
